...
<div class="content-wrapper">
    ...
    @include('patial.content-header',['name'=>'Setting', 'key'=>'Edit'])
    <!-- /.content-header -->
                ...
                <form action="{{ route('setting.update',['id'=>$settings->id]) }}" method="post">
                  @csrf
                    <div class="form-group">
                      ...
                      <input type="text" name="config_key" value="{{$settings->config_key}}"
                      class="form-control @error('config_key') is-invalid @enderror"   placeholder="Config key">
                    </div>
                    @if (request() ->type === 'Text')
                    <div class="form-group">
                        <label >Config value</label>
                        <input type="text" name="config_value"  value="{{$settings->config_value}}"
                    ...
                    @elseif(request() ->type === 'Textarea')
                      <div class="form-group">
                        ...
                        <textarea name="config_value"  class="form-control @error('config_value') is-invalid @enderror" cols="30"
                         rows="5" placeholder="Config value" >{{$settings->config_value}}</textarea>
                      </div> 
                    @endif
